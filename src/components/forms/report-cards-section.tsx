...
function ReportCardRow({
  ...
  async function handleDelete() {
    if (!confirm(`Delete "${rc.periodLabel}"? This cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      alert("Something went wrong while deleting the report card.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <li className="flex items-center justify-between px-4 py-3">
      <div>
        ...
        <p className="text-xs text-slate-500">
          Generated{" "}
          {new Date(rc.generatedAt).toLocaleDateString("en-GB")}
        </p>
      </div>
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch(
        `/api/students/${studentId}/report-cards`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        setError(
          typeof data.error === "string"
            ? data.error
            : "Could not generate the report card."
        );
        return;
      }
      ...
      setError(
        "Something went wrong while generating the report card."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-xs text-slate-400">
        Leave From/To blank to include every mark on record.
        Attendance defaults to the standard 30-day window when no
        range is given.
      </p>
